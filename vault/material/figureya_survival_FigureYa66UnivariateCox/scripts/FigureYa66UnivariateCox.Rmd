...
计算HR，保存到文件，作为FigureYa6rHRs的输入。

Single gene cox analysis and plot like in PAPER.

Calculate HR and save to file as input for FigureYa6rHRs.

![](example.png)

Fig. 3. Forrest plots of hazard ratios of survival associated alternative events in LUSC. A, Hazard ratios of top 20 genes with survival associated ES events. B, Hazard ratios of top 20 genes with survival associated RI events. C, Hazard ratios of top 20 genes with survival associated AP events. D, Hazard ratios of top 20 genes with survival associated AT events. E, Hazard ratios of top 20 genes with survival associated AD events. F, Hazard ratios of top 20 genes with survival associated AA events. G, Hazard ratios of 8 genes with survival associated ME events.
...
Univariate Cox analysis is used to explore the independent prognostic impact of individual genes.
...
library(survival) # Cox回归分析/Cox regression analysis
library(stringr) # 字符串处理/string processing
library(viridis) # 颜色处理/color processing
library(scales) # 颜色处理/color processing
...
If your data has been organized into easy_input_4coxv.csv format, you can skip this step and go directly to "Univariate Cox analysis".
...
### 第一步：样本资料整理，提取LUAD样本名 Step 1: Sample information sorting, extract LUAD sample names

数据来源：

- TCGA_phenotype_denseDataOnlyDownload.tsv，TCGA样本资料，下载自：<https://pancanatlas.xenahubs.net/download/TCGA_phenotype_denseDataOnlyDownload.tsv.gz>
...
- Survival_SupplementalTable_S1_20171025_xena_sp，cell处理优化过的生存资料，下载自：<https://pancanatlas.xenahubs.net/download/Survival_SupplementalTable_S1_20171025_xena_sp.gz>

Data Source:

- TCGA_phenotype_denseDataOnlyDownload.tsv, TCGA sample data, download from <https://pancanatlas.xenahubs.net/download/TCGA_phenotype_denseDataOnlyDownload.tsv.gz>
...
- Survival_SupplementalTable_S1_20171025_xena_sp, cell processing optimized survival information, download from <https://pancanatlas.xenahubs.net/download/Survival_ SupplementalTable_S1_20171025_xena_sp.gz>
...
### 第二步：表达矩阵处理 Step 2: Expression matrix processing
...
### 第三步：整理为cox分析的输入格式 Step 3: Organize into an input format for cox analysis
...
## 单因素cox分析 Univariate Cox analysis
...
此结果可与FigureYa6rHRs衔接，画出不同形式的森林图

只需把FigureYa6rHRs输入文件的Point Estimate改为HR，Low改为lower，High改为upper，其余自行添加

This result can be linked to the FigureYa6rHRs to draw a different form of forest plot.

Just change Point Estimate to HR, Low to lower, High to upper in the input file of FigureYa6rHRs, and add the rest by yourself.
...
## 例文的原图复现 Original image reproduction of example text
