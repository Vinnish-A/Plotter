...
The author used two non-proportional risk prognostic models to calculate the difference in survival in the cohort that received immunotherapy, including RMS-limiting mean survival and long-term survival analysis. This is very meaningful in clinical practice, because immunotherapy patients do have the "delayed effect".

![](example.jpg)

From <https://onlinelibrary.wiley.com/doi/10.1002/ctm2.601> Supplementary Figure S4a
...
Xiaofan Lu wrote a magical R package MOVICS, ran his own data, and the key Figure in the article came out.

How to organize these pictures together to tell a story? This article is a good example of applying
...
library(survRM2) # 用于计算RMS限制性生存均值 # Used to calculate RMS limiting mean survival
library(ComparisonSurv) # 用于计算long-term survival长期生存模型 # Used to calculate long-term survival model
library(ggplot2) # 画图包 # Drawing package
library(ggpubr) # 画图包 # Drawing package
library(survminer) # 画生存曲线包 # Draw survival curve package
...
## 采用非比例风险的生存模型（non-proportional-hazard survival model, NPHSurv）评价接受免疫治疗患者的预后情况 The non-proportional-hazard survival model (NPHSurv) was used to evaluate the prognosis of patients receiving immunotherapy
...
这里只绘制基本图形，细节均需要Illustrator补充。

Only basic graphics are drawn here, and the details need to be supplemented by Illustrator.
